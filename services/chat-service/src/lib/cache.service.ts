	...
	private initialize() {
		try {
			...
			this.redis.on("error", (err) => {
				this.enabled = false;
				this.logger?.warn?.(
					"[chat-service] Redis connection error - caching disabled",
					err,
				);
			});
			...
			this.redis.connect().catch((err) => {
				this.enabled = false;
				this.logger?.warn?.(
					"[chat-service] Redis connection failed - caching disabled",
					err,
				);
			});
		} catch (error) {
			this.logger?.warn?.(
				"[chat-service] Redis initialization failed - caching disabled",
				error,
			);
			this.enabled = false;
		}
