...
const conversationsRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.addHook("preValidation", async (request, reply) => {
    ...
    const body = request.body as { targetUserId?: string; targetEmail?: string };
    ...
    if (!body || typeof body !== "object" || (!body.targetUserId && !body.targetEmail)) {
      ...
      if (!targetId && body.targetEmail) {
        try {
          const res = await (fastify as any).db.execute(sql`SELECT id FROM users WHERE email = ${body.targetEmail} LIMIT 1`);
          targetId = (res.rows[0] as { id: string } | undefined)?.id;
        } catch (lookupError) {
      ...
      const getOrCreateTeamchatIdByEmail = async (email: string): Promise<string> => {
        const [existing] = await db
          .select({ id: teamchatUsers.id })
          .from(teamchatUsers)
      ...
      const body = (request.body ?? null) as unknown;
