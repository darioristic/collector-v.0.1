		...
		socket.on(
			...
			async (_payload: { userId?: string }) => {
				try {
					...
					const users = (companyUsers.rows as Array<{ userId: string; status: string }>).map(
						(row) => ({ userId: row.userId, status: (row.status as "online" | "offline" | "away") || "offline" }),
					);

					// Emit only to the requesting socket
					socket.emit("users:online", { users });
					console.log(
						`[chat-service] Sent users:online list (${users.length}) to user ${user.userId}`,
					);
				} catch (error) {
