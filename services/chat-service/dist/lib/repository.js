...
import { chatConversations, chatMessages, } from "../db/schema/chat.js";
 ...
 */
async function ensureTeamchatUsers(userIds, companyId) {
    if (userIds.length === 0) {
        return;
    }
    try {
        ...
        const existingUsers = await db
            ...
            .from(teamchatUsers)
            .where(and(eq(teamchatUsers.companyId, companyId), inArray(teamchatUsers.id, userIds)));
        ...
        `);
        const foundIds = new Set(usersData.rows.map((r) => r.id));
        ...
        for (const userData of usersData.rows) {
...
const getConversationCacheKey = (companyId, userId1, userId2) => `${CACHE_PREFIX}conversation:${companyId}:${userId1 < userId2 ? userId1 : userId2}:${userId1 < userId2 ? userId2 : userId1}`;
const getConversationsListCacheKey = (companyId, userId) => `${CACHE_PREFIX}conversations:${companyId}:${userId}`;
const getUserStatusCacheKey = (companyId, userId) => `${CACHE_PREFIX}user:status:${companyId}:${userId}`;
const getChannelMembershipCacheKey = (companyId, userId) => `${CACHE_PREFIX}channels:${companyId}:${userId}`;
// Global cache instance (will be set by server)
let globalCache = null;
export const setCacheService = (cache) => {
    globalCache = cache;
};
export const findOrCreateConversation = async ({ companyId, currentUserId, targetUserId, cache, }) => {
    const cacheService = cache || globalCache;
    // Resolve teamchat user IDs for both users (maps users.id -> teamchat_users.id)
    const resolveTeamchatId = async (userId) => {
    ...
    if (existing) {
        ...
        const [unreadResult] = await db
            ...
            .from(chatMessages)
            .where(and(eq(chatMessages.conversationId, existing.id), ne(chatMessages.senderId, currentUserId), isNull(chatMessages.readAt)));
        ...
        if (cacheService) {
            ...
            await cacheService.delete(getConversationsListCacheKey(companyId, currentUserId), getConversationsListCacheKey(companyId, targetUserId));
        }
        return result;
    }
    const now = new Date();
...
export const getConversations = async ({ companyId, userId, cache, }) => {
    const cacheService = cache || globalCache;
    // Resolve teamchat user id for the current user
    const resolveTeamchatId = async (id) => {
        const [byId] = await db
            .select({ id: teamchatUsers.id })
            .from(teamchatUsers)
    ...
    const conversationIds = await db
        ...
        .from(chatConversations)
        .where(and(eq(chatConversations.companyId, companyId), or(eq(chatConversations.userId1, teamchatUserId), eq(chatConversations.userId2, teamchatUserId))));
    ...
    const unreadCounts = conversationIdsList.length > 0
        ? await db
            ...
            .from(chatMessages)
            .where(and(inArray(chatMessages.conversationId, conversationIdsList), ne(chatMessages.senderId, teamchatUserId), isNull(chatMessages.readAt)))
...
export const createMessage = async ({ conversationId, senderId, content, type = "text", fileUrl, fileMetadata, cache, }) => {
    const cacheService = cache || globalCache;
    const now = new Date();
    ...
    if (cacheService) {
        ...
        if (conv) {
            ...
            await cacheService.delete(getConversationCacheKey(conv.companyId, userId1, userId2), getConversationsListCacheKey(conv.companyId, conv.userId1), getConversationsListCacheKey(conv.companyId, conv.userId2));
        }
    }
    return result;
};
...
export const getConversationById = async ({ conversationId, userId, }) => {
    ...
    const [row] = await db
        ...
        .innerJoin(user1Alias, eq(chatConversations.userId1, user1Alias.id))
        .innerJoin(user2Alias, eq(chatConversations.userId2, user2Alias.id))
        .where(and(eq(chatConversations.id, conversationId), or(eq(chatConversations.userId1, teamchatUserId), eq(chatConversations.userId2, teamchatUserId))))
        .limit(1);
    if (!row) {
        return null;
    }
    ...
    const [unreadResult] = await db
        ...
        .from(chatMessages)
        .where(and(eq(chatMessages.conversationId, row.id), ne(chatMessages.senderId, teamchatUserId), isNull(chatMessages.readAt)));
    const unreadCount = Number(unreadResult?.count) || 0;
    return {
        id: row.id,
        userId1: row.userId1,
