...
export async function authMiddleware(request, reply) {
    ...
    try {
        request.log.debug({ tokenLength: token.length }, "Validating session token");
        ...
        if (!session) {
            request.log.warn({ tokenLength: token.length, tokenPrefix: token.substring(0, 10) }, "Session not found or expired");
