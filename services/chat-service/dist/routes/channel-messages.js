import { createMessage as createTeamChatMessage, getChannelMessages, } from "../lib/teamchat-repository.js";
import { authMiddleware } from "../lib/auth.js";
const channelMessagesRoutes = async (fastify) => {
    ...
    fastify.get("/", async (request, reply) => {
        ...
        try {
            ...
            const messages = await getChannelMessages(channelId, request.user.userId, limit, cache);
    ...
    fastify.post("/", async (request, reply) => {
        ...
        try {
            const cache = fastify.cache;
            const message = await createTeamChatMessage(body.channelId, request.user.userId, body.content || null, body.fileUrl || null, cache);
            // Emit socket event
            const io = fastify.io;
            if (io) {
