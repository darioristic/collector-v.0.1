...
const getUserStatusCacheKey2 = (companyId, userId) => `${CACHE_PREFIX}user:status:${companyId}:${userId}`;
    ...
    fastify.get("/:id/status", async (request, reply) => {
        ...
        try {
            ...
            const [userRecord] = await db
                ...
                .from(teamchatUsers)
                .where(and(eq(teamchatUsers.id, userId), eq(teamchatUsers.companyId, user.companyId)))
                .limit(1);
            if (!userRecord) {
        ...
        try {
            ...
            if (cache) {
                for (let i = 0; i < body.userIds.length; i++) {
                    const cached = await cache.get(cacheKeys[i]);
                    if (cached) {
            ...
            const users = uncachedIds.length > 0 ? await db
                ...
                .from(teamchatUsers)
                .where(and(eq(teamchatUsers.companyId, user.companyId), inArray(teamchatUsers.id, uncachedIds))) : [];
