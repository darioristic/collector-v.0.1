...
const statusLabelMap: Record<
	(typeof projectStatus.enumValues)[number],
	string
> = {
	planned: "Pending",
	active: "Active",
...
const timelineStatusToTaskStatus: Record<
	TimelineStatus,
	(typeof taskStatus.enumValues)[number]
> = {
	completed: "done",
...
const taskStatusToTimelineStatus: Record<
	(typeof taskStatus.enumValues)[number],
	TimelineStatus
> = {
	todo: "upcoming",
...
export class ProjectsService {
	...
	async list(): Promise<ProjectSummary[]> {
		if (this.cache) {
			const cached = await this.cache.get<ProjectSummary[]>(
		...
		const taskStats = this.database
			.select({
				...
				completedCount:
					sql<number>`sum(case when ${projectTasks.status} = 'done' then 1 else 0 end)`.as(
						"completedCount",
					),
			})
			.from(projectTasks)
		...
		const summaries = rows.map(
			({ project, owner, totalTasks, completedTasks }) =>
	...
	async createProject(input: ProjectCreateInput): Promise<ProjectDetails> {
		const payload: typeof projects.$inferInsert = {
			...
			budgetTotal:
				input.budget?.total != null ? input.budget.total.toString() : null,
			budgetSpent:
				input.budget?.spent != null ? input.budget.spent.toString() : null,
		...
		try {
			...
			for (const taskInput of taskTemplates) {
				try {
					...
					console.error(
						`Failed to create default task "${taskInput.title}" for project ${row.id}:`,
						error,
					);
				}
			}
			...
			// Log error but don't fail project creation
			console.error(
				`Failed to create default tasks for project ${row.id}:`,
				error,
			);
		}
	...
	async updateProject(
		...
		if (input.startDate !== undefined)
			payload.startDate = toDateOrNull(input.startDate);
		if (input.dueDate !== undefined)
			payload.dueDate = toDateOrNull(input.dueDate);
	...
	async getProjectDetails(id: string): Promise<ProjectDetails | null> {
		...
		const [tasks, timeline, teams, team, budgetCategories, timeEntries] =
			await Promise.all([
		...
		const summary = this.mapProjectSummary(
			projectRow.project,
			projectRow.owner ?? undefined,
			{
				total: totalTasks,
		...
		const budget = this.composeBudgetSummary(
			projectRow.project,
			budgetCategories,
			timeEntries,
		);
	...
	async deleteTask(projectId: string, taskId: string): Promise<boolean> {
		const deleted = await this.database
			.delete(projectTasks)
			.where(
				and(eq(projectTasks.id, taskId), eq(projectTasks.projectId, projectId)),
			)
			.returning();
		const success = deleted.length > 0;
	...
	async deleteTimelineEvent(
		...
		const deleted = await this.database
			...
			.where(
				and(
					eq(projectMilestones.id, eventId),
	...
	async updateTeam(
		...
		const existing = await this.database
			...
			.from(projectTeams)
			.where(
				and(eq(projectTeams.id, teamId), eq(projectTeams.projectId, projectId)),
			)
			.limit(1);
	...
	async addTeamMember(
		...
		if (input.teamId) {
			const [team] = await this.database
				...
				.where(
					and(
						eq(projectTeams.id, input.teamId),
	...
	async updateBudget(
		projectId: string,
		input: UpdateBudgetSummaryInput,
		...
		const payload: typeof projectBudgetCategories.$inferInsert = {
			...
			allocatedAmount:
				input.allocated !== undefined && input.allocated !== null
					? input.allocated.toString()
					: "0",
			spentAmount:
				input.spent !== undefined && input.spent !== null
					? input.spent.toString()
					: "0",
		};
	...
	async updateBudgetCategory(
		...
		const existing = await this.database
			...
			.where(
				and(
					eq(projectBudgetCategories.id, categoryId),
					eq(projectBudgetCategories.projectId, projectId),
				),
			)
			.limit(1);
	...
	async updateTimeEntry(
		...
		if (input.date !== undefined)
			payload.date = toDateOrNull(input.date) ?? new Date();
		if (input.description !== undefined)
	...
	private composeBudgetSummary(
		...
		const categoriesAllocated = categories.reduce(
			(acc, item) => acc + item.allocated,
			0,
		);
		const categoriesSpent = categories.reduce(
	...
	private async fetchProjectTasks(projectId: string): Promise<ProjectTask[]> {
		...
		return rows.map(({ task, assignee }) =>
			this.mapTask(task, assignee ?? undefined),
		);
	}
	...
	private mapTask(
		task: TasksTableRow,
		assignee?: BasicUserInfo | UserRow,
	): ProjectTask {
		return {
			id: task.id,
	...
	private async fetchProjectTimeline(
		projectId: string,
	): Promise<ProjectTimelineEvent[]> {
		if (!isUuid(projectId)) {
			return [];
	...
	private async getTimelineEventById(
		eventId: string,
	): Promise<ProjectTimelineEvent | null> {
		if (!isUuid(eventId)) {
			return null;
	...
	private async fetchProjectTeam(
		projectId: string,
	...
	private async fetchProjectTimeEntries(
		projectId: string,
	): Promise<ProjectTimeEntry[]> {
		if (!isUuid(projectId)) {
			return [];
		...
		const rows = await this.database
			...
			.orderBy(
				desc(projectTimeEntries.date),
				desc(projectTimeEntries.createdAt),
			);

		return rows.map(({ entry, user, task }) =>
			this.mapTimeEntry(entry, user ?? undefined, task ?? undefined),
		);
	}

	private async getTimeEntryById(
		entryId: string,
	): Promise<ProjectTimeEntry | null> {
		if (!isUuid(entryId)) {
			return null;
	...
	private async fetchBudgetCategories(
		projectId: string,
	): Promise<ProjectBudgetCategory[]> {
		if (!isUuid(projectId)) {
			return [];
