...
export default function ForgotPasswordPage() {
	...
	const onSubmit = async (values: FormValues) => {
		...
		if (!response.ok) {
			const message =
				payload?.message ?? "Slanje instrukcija nije uspelo. Pokušajte ponovo.";
			toast.error(message);
			return;
		}
		...
		<div className="flex items-center justify-center py-6 lg:h-screen">
			<Card className="mx-auto w-full max-w-md">
				<CardHeader>
					...
					<CardDescription>
						Unesite email adresu i poslaćemo vam privremeni token za reset
						lozinke. Pošto je okruženje demo, token će se prikazati ispod forme.
					</CardDescription>
				</CardHeader>
				<CardContent>
					<Form {...form}>
