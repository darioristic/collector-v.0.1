...
export async function PUT(_request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const folder = await db
    ...
    .from(vaultFolders)
    .where(eq(vaultFolders.id, params.id))
    .limit(1)
  ...
  if (folder) {
    ...
    if (updatedFolder.createdBy) {
      const [ownerRow] = await db
        ...
        .from(teamMembers)
        .where(eq(teamMembers.id, updatedFolder.createdBy))
        .limit(1);
  ...
  if (file) {
    ...
    if (updatedFile.uploadedBy) {
      const [ownerRow] = await db
        ...
        .from(teamMembers)
        .where(eq(teamMembers.id, updatedFile.uploadedBy))
        .limit(1);
...
export async function DELETE(_request: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (folder) {
    ...
    while (stack.length > 0) {
      ...
      if (foldersToDelete.has(current)) {
        continue;
      }
    ...
    await db.transaction(async (tx) => {
      await tx.delete(vaultFolders).where(inArray(vaultFolders.id, foldersToDeleteArray));
    });
