...
export async function POST(request: NextRequest) {
  ...
  const parsed = formSchema.safeParse({
    ...
    uploadedBy:
      typeof uploadedByValue === "string" && uploadedByValue.length > 0 ? uploadedByValue : undefined
  });
  ...
  if (parsed.data.uploadedBy) {
    const [member] = await db
      ...
      .from(teamMembers)
      .where(eq(teamMembers.id, parsed.data.uploadedBy))
      .limit(1);
  ...
  try {
    ...
    for (const file of files) {
      ...
      if (file.size > MAX_FILE_SIZE_BYTES) {
        throw new Error(`Fajl ${file.name} prelazi maksimalnu dozvoljenu veličinu od 50MB.`);
      }
    ...
    const message = error instanceof Error ? error.message : "Otpremanje fajla nije uspelo.";
