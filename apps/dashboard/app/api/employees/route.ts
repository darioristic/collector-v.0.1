...
const buildOrderBy = (
	sortField: ListEmployeesQuery["sortField"],
	sortOrder: SortOrder,
) => {
	...
	if (sortField === "startDate") {
		return [
			isAsc ? asc(employees.startDate) : desc(employees.startDate),
			isAsc ? asc(employees.id) : desc(employees.id),
		];
	}

	return [
		isAsc ? asc(employees.status) : desc(employees.status),
		isAsc ? asc(employees.id) : desc(employees.id),
	];
};
...
const buildCursorCondition = (cursor: EmployeeCursor): SQL => {
	...
	if (cursor.sortField === "name") {
		const condition = or(
			isAsc
				? gt(employees.lastName, cursor.lastName)
				: lt(employees.lastName, cursor.lastName),
			and(
				eq(employees.lastName, cursor.lastName),
				isAsc
					? gt(employees.firstName, cursor.firstName)
					: lt(employees.firstName, cursor.firstName),
			)!,
			and(
				eq(employees.lastName, cursor.lastName),
	...
	if (cursor.sortField === "startDate") {
		...
		const condition = or(
			isAsc
				? gt(employees.startDate, cursorDate)
				: lt(employees.startDate, cursorDate),
			and(
	...
	const condition = or(
		isAsc
			? gt(employees.status, cursor.status)
			: lt(employees.status, cursor.status),
		and(
			eq(employees.status, cursor.status),
			isAsc ? gt(employees.id, cursor.id) : lt(employees.id, cursor.id),
		)!,
	);
...
const buildSearchConditions = (query: ListEmployeesQuery) => {
	...
	if (query.employmentType) {
		clauses.push(
			eq(
				...
				employmentTypeSchema.parse(query.employmentType),
			),
		);
	}
	...
	if (query.status) {
		clauses.push(
			eq(employees.status, employmentStatusSchema.parse(query.status)),
		);
	}
...
export async function GET(request: NextRequest) {
	...
	if (query.cursor) {
		...
		if (
			!cursor ||
			cursor.sortField !== query.sortField ||
	...
	const filteredStatement =
		filters.length > 0
			? baseStatement.where(filters.length === 1 ? filters[0] : and(...filters))
			: baseStatement;

	const orderBy = buildOrderBy(query.sortField, query.sortOrder);

	const rows = await filteredStatement
		.orderBy(...orderBy)
		.limit(query.limit + 1);
...
export async function POST(request: NextRequest) {
	...
	try {
		...
		return withNoStore(
			NextResponse.json({ data: serializeEmployee(employee) }, { status: 201 }),
		);
	} catch (error) {
		const message =
