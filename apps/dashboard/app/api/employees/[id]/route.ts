...
export async function PUT(request: NextRequest, context: EmployeeRouteContext) {
	...
	if (updateInput.salary !== undefined) {
		updatePayload.salary = updateInput.salary.toString();
	}
	...
	try {
		...
		return withNoStore(
			NextResponse.json({ data: serializeEmployee(updatedEmployee) }),
		);
	} catch (error) {
		const message =
...
export async function DELETE(_: NextRequest, context: EmployeeRouteContext) {
	const { id: rawId } = await extractParams(context);
	const parsedId = employeeIdSchema.safeParse({ id: rawId });
