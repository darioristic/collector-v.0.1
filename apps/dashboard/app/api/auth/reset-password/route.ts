...
export async function POST(request: NextRequest) {
  ...
  const { response, payload } = await forwardAuthRequest<AuthBackendResponse>("reset-password", {
    method: "POST",
    body: JSON.stringify(body)
  });

  if (!response.ok) {
    return backendErrorResponse(response, payload, "AUTH_RESET_FAILED", "Reset lozinke nije uspeo.");
  }
