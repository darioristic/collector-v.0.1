...
export async function POST(request: NextRequest) {
  ...
  const { response, payload } = await forwardAuthRequest<AuthBackendResponse>("login", {
    method: "POST",
    body: JSON.stringify(body)
  });

  if (!response.ok) {
    return backendErrorResponse(response, payload, "AUTH_LOGIN_FAILED", "Prijava nije uspela.");
  }
