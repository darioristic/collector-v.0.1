...
export async function POST(request: NextRequest) {
  ...
  const { response, payload } = await forwardAuthRequest<AuthBackendResponse>("register", {
    method: "POST",
    body: JSON.stringify(body)
  });

  if (!response.ok) {
    return backendErrorResponse(response, payload, "AUTH_REGISTER_FAILED", "Registracija nije uspela.");
  }
