...
import { renderInvoiceToBuffer, renderInvoiceToStream } from "@/lib/invoice-pdf/renderer";
export const runtime = "nodejs";
export const dynamic = "force-dynamic";
import type { InvoicePDFProps, EditorDoc, EditorNode, EditorInlineContent } from "@/lib/invoice-pdf/types";
...
export async function GET(req: NextRequest) {
  ...
  if (!id) {
    ...
    const pdfProps: InvoicePDFProps = {
      ...
      try {
        ...
        try {
          ...
          const { renderMinimalToBuffer } = await import("@/lib/invoice-pdf/renderer");
          const msg = `PDF error: ${String(e ?? "unknown")} | ${String(ee ?? "unknown")}`.slice(0, 160);
    ...
    const title = typeof error === "object" && error ? "Invoice PDF Error" : undefined;
