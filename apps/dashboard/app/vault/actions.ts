...
export async function getVaultContentsAction(params?: {
	...
	const response = await fetch(
		`/api/vault${queryString ? `?${queryString}` : ""}`,
		{
			method: "GET",
...
export async function uploadVaultFilesAction(
	formData: FormData,
...
export async function renameVaultItemAction(input: {
	id: string;
	name: string;
}): Promise<
	(VaultFolderItem & { type: "folder" }) | (VaultFileItem & { type: "file" })
> {
...
export async function deleteVaultItemAction(id: string): Promise<{
	...
	const result = await handleResponse<{
		data: { deletedFolders?: string[]; deletedFiles?: string[] };
	}>(response);
	revalidateVault();
	return {
