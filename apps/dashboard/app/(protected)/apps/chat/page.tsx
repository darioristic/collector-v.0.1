...
import { ChatSidebar, ChatContent } from "@/app/(protected)/apps/chat/components";
...
async function getChats() {
  const data = await fs.readFile(
    path.join(process.cwd(), "app/(protected)/apps/chat/data/chats.json")
  );
...
async function getChatUser(id: number) {
  const data = await fs.readFile(
    path.join(process.cwd(), "app/(protected)/apps/chat/data/contacts.json")
  );
