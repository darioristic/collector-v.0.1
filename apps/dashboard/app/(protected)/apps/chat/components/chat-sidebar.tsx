...
import useChatStore, { type ChatItem } from "@/app/(protected)/apps/chat/useChatStore";
import { fetchTeamMembers, type TeamMember } from "@/app/(protected)/settings/teams/api";
...
export function ChatSidebar() {
  ...
  const [chatServiceUnavailable, setChatServiceUnavailable] = React.useState(false);
  ...
  const isChatServiceUnavailable =
    ...
    (conversationsQuery.isError &&
      ...
      (conversationsQuery.error.message.toLowerCase().includes("servis nije dostupan") ||
        conversationsQuery.error.message.toLowerCase().includes("service unavailable") ||
        conversationsQuery.error.message.toLowerCase().includes("nije dostupan") ||
        conversationsQuery.error.message.toLowerCase().includes("failed to fetch")));
  ...
  const filteredConversations = React.useMemo(() => {
    ...
    return conversations.filter((conv) => {
      ...
      const displayName =
        ...
        otherUser.email;
      return (
        displayName.toLowerCase().includes(term) || otherUser.email.toLowerCase().includes(term)
      );
    });
  ...
  const chatItems = React.useMemo(() => {
    ...
    return filteredConversations.map((conv) => {
      ...
      const onlineStatus: "success" | "warning" | "danger" =
        status === "online" ? "success" : status === "away" ? "warning" : "danger";
      ...
      return {
        ...
        user: {
          ...
          last_seen: status === "online" ? undefined : status === "away" ? "Away" : "Offline"
        },
        date,
  ...
  const createConversationMutation = useMutation({
    mutationFn: async (member: TeamMember) => {
      ...
      try {
        console.log("[chat] Calling createConversation with userId:", member.userId);
        ...
        if (isServiceUnavailable) {
          ...
          console.warn("[chat] Chat service unavailable when creating conversation", {
            userId: member.userId,
            errorMessage
          });
          // Throw clear error that will be handled by onError callback
          throw new Error("Chat servis nije dostupan. Proverite da li je servis pokrenut.");
        }
    ...
    onSuccess: (conversation) => {
      ...
      setTimeout(() => {
        ...
        console.log("[chat] Verified selected chat after set:", {
          ...
          matches: store.selectedChat?.conversationId === newChatItem.conversationId
        });
      ...
      queryClient.setQueryData<ChatConversation[]>(
        ...
        (oldConversations = []) => {
          ...
          if (exists) {
            ...
            return oldConversations.map((c) => (c.id === conversation.id ? conversation : c));
          }
  ...
  React.useEffect(() => {
    ...
    if (
      ...
      if (firstChat?.conversationId && firstChat?.otherUser) {
        console.log("[chat] Auto-selecting first conversation:", firstChat.conversationId);
        setSelectedChat({
    ...
    <Card className="w-full pb-0 lg:w-96">
      ...
      <CardContent className="flex-1 overflow-auto p-0">
        <div className="block min-w-0 divide-y">
          {isChatServiceUnavailable && (
            <div className="bg-muted/50 border-muted-foreground/20 m-4 rounded-lg border p-4">
              ...
              <div className="text-muted-foreground text-xs">
                Chat servis nije pokrenut. Da biste koristili chat, pokrenite servis sa{" "}
                <code className="bg-muted rounded px-1 py-0.5">bun run dev</code> u root
          ...
          {chatItems.length > 0 &&
            chatItems.map((chat) => {
              const originalConversation = conversations.find((c) => c.id === chat.conversationId);
              ...
              return (
                <ChatListItem
                  ...
                  active={selectedChat && selectedChat.conversationId === chat.conversationId}
                />
              );
            })}
            ...
            <>
              ...
              <div className="bg-muted/30 px-6 py-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-muted-foreground text-sm font-medium">Kolege iz tima</span>
                </div>
              </div>
              {filteredTeamMembers.map((member) => {
                // Get real-time status from socket, prioritize real-time over database status
                const realTimeStatus = member.userId ? getUserStatus(member.userId) : null;
                // If real-time status exists, use it; otherwise use database status
                const memberStatus = realTimeStatus ?? (member.status || "offline");
                ...
                const isCreating =
                  ...
                  (createConversationMutation.variables?.id === member.id ||
                    createConversationMutation.variables?.userId === member.userId);
                ...
                return (
                  <button
                    ...
                    disabled={!hasUserId || isCreating || isChatServiceUnavailable}>
                    <Avatar className="overflow-visible md:size-10">
                      <AvatarImage src={member.avatarUrl || undefined} alt="avatar image" />
                      <AvatarIndicator variant={statusColor} />
                      <AvatarFallback>{generateAvatarFallback(member.fullName)}</AvatarFallback>
                    </Avatar>
                    <div className="min-w-0 grow">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {!hasUserId && (
                          <span className="text-muted-foreground text-xs">(Nije registrovan)</span>
                        )}
                      </div>
                    </div>
                    {isCreating && (
              ...
              <div className="text-muted-foreground mt-4 space-y-2 px-6 text-center text-sm">
                ...
                {teamMembers.length > 0 ? (
                  <div className="space-y-1 text-xs">
                    <div>
                      Pronađeno {teamMembers.length} članova tima, ali nijedan nije dostupan za
                      chat.
                    </div>
                    {teamMembers.filter((m) => !m.userId).length > 0 && (
                      <div className="text-amber-600 dark:text-amber-400">
                        {teamMembers.filter((m) => !m.userId).length} član(ova) nije registrovan u
                        sistemu. Registrujte ih da biste mogli da započnete chat.
                      </div>
                    )}
                    {teamMembers.filter((m) => m.userId).length > 0 && (
                      <div>
                        {teamMembers.filter((m) => m.userId).length} član(ova) je već u
                        konverzacijama.
                      </div>
                    )}
                  </div>
