...
export function ChatContent() {
  ...
  const myTeamchatUserId = React.useMemo(() => {
    if (!selectedChat?.user1 || !selectedChat?.user2 || !currentUserEmail) return null;
    const u1Email = selectedChat.user1.email?.toLowerCase() || null;
    const u2Email = selectedChat.user2.email?.toLowerCase() || null;
  ...
  useEffect(() => {
    ...
    const unsubscribeMessage = onNewMessage((payload) => {
      ...
      const idsToCheck = [myTeamchatUserIdRef.current, currentUserId]
        .map((v) => normalizeId(v))
        .filter((v) => v !== "");
      const isOwnMessage = idsToCheck.length > 0 && idsToCheck.includes(senderId);
            ...
            {messagesQuery.isError && (
              <div className="text-destructive w-full text-center text-sm">
                ...
                {messagesQuery.error instanceof Error && (
                  <p className="text-xs opacity-75">{messagesQuery.error.message}</p>
                )}
              </div>
            )}
            {isOtherUserTyping && (
              <div className="flex w-full justify-start">
                <div className="bg-muted flex max-w-[70%] items-center gap-2 rounded-md border p-3">
                  ...
                  <span className="text-muted-foreground text-xs">
                    {otherUser?.firstName || otherUser?.email || "Korisnik"} kuca...
                  </span>
                </div>
              </div>
            )}
            {messages.length > 0 &&
              messages.map((message: ChatMessage) => {
                ...
                if (isOptimistic) {
                  ...
                  const idsToCheck = [myComparableUserId, currentUserId ? String(currentUserId).trim() : ""].filter((v) => v !== "");
                    ...
                    message={{
                      ...
                      data: message.fileUrl
                        ? {
                            ...
                            images: message.type === "image" ? [message.fileUrl] : undefined,
                            cover: message.type === "video" ? message.fileUrl : undefined
                          }
                        : undefined
                    }}
                    type={message.type}
