...
export default function VaultPage() {
  ...
  }, [debouncedSearch, currentFolderId, pathname, router, searchParams, searchParamsString]);
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/vault${params.toString() ? `?${params}` : ""}`, {
          method: "GET",
          headers: {
        ...
        const body = (await response.json()) as VaultListingResponse & { error?: string };

        if (!response.ok) {
          throw new Error(body?.error ?? "Neuspešno učitavanje Vault sadržaja.");
        }
        ...
        toast({
          title: "Greška",
          description: normalizeErrorMessage(error, "Neuspešno učitavanje podataka."),
          role: "alert"
        });
      } finally {
        if (isMounted) {
  ...
  const handleCreateFolderSubmit = () => {
    ...
    startCreating(async () => {
      try {
        ...
        toast({
          title: "Greška",
          description: normalizeErrorMessage(error, "Kreiranje foldera nije uspelo.")
        });
      }
    });
  };
  ...
  const handleRenameSubmit = () => {
    ...
    startRenaming(async () => {
      try {
        ...
        toast({
          title: "Greška",
          description: normalizeErrorMessage(error, "Preimenovanje nije uspelo.")
        });
      }
    });
  };
  ...
  return (
    <>
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={(open) => (!open ? setCreateDialogOpen(false) : setCreateDialogOpen(true))}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Novi folder</DialogTitle>
            <DialogDescription>Unesite naziv novog foldera koji želite da kreirate.</DialogDescription>
          </DialogHeader>
          <Input
            value={newFolderName}
          ...
          <DialogFooter>
            ...
            <Button type="button" onClick={handleCreateFolderSubmit} disabled={isCreating}>
      ...
      <Dialog open={Boolean(renameTarget)} onOpenChange={(open) => (!open ? setRenameTarget(null) : null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Preimenuj {renameTarget?.kind === "folder" ? "folder" : "fajl"}</DialogTitle>
            <DialogDescription>Unesite novi naziv i sačuvajte promene.</DialogDescription>
          </DialogHeader>
          <Input
            value={renameValue}
          ...
          <DialogFooter>
            ...
            <Button type="button" onClick={handleRenameSubmit} disabled={isRenaming}>
      ...
      <AlertDialog open={Boolean(deleteTarget)} onOpenChange={(open) => (!open ? setDeleteTarget(null) : null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Obriši {deleteTarget?.kind === "folder" ? "folder" : "fajl"}</AlertDialogTitle>
            <AlertDialogDescription>
              {deleteTarget?.kind === "folder"
