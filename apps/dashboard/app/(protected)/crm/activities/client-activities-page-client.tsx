...
import { CalendarDays, Filter, LayoutList, Plus, Table as TableIcon } from "lucide-react";
...
import { createClientActivity, deleteClientActivity, updateClientActivity } from "./data";
import { type ActivityModalMode, type ActivityView, useActivitiesStore } from "./store";
...
export function ClientActivitiesPageClient({
  ...
  const currentViewOption = viewOptions.find((option) => option.value === view) ?? viewOptions[0];
    ...
    <div className="space-y-8">
            ...
            <DropdownMenuContent align="end" className="w-48">
              {viewOptions.map((option) => (
                <DropdownMenuItem
                  ...
                  className={cn(option.value === view ? "text-primary font-semibold" : "")}>
        ...
        <SheetContent className="w-full sm:max-w-xl">
          ...
          <div className="mt-6 space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-1.5">
                <label htmlFor={filterAssignedToId} className="text-sm font-medium">
                  Assigned To
                </label>
                <Select
                  ...
                  onValueChange={(value) =>
                    setFilters({
                      assignedTo: value === ALL_FILTER_VALUE ? undefined : value
                    })
                  }>
              ...
              <div className="space-y-1.5">
                <label htmlFor={filterPriorityId} className="text-sm font-medium">
                  Priority
                </label>
                <Select
      ...
      <Dialog open={isModalOpen} onOpenChange={(open) => (!open ? setModal(false) : null)}>
        <DialogContent className="max-h-[95vh] overflow-y-auto sm:max-w-xl">
          <DialogHeader>
            <DialogTitle>{modalMode === "edit" ? "Edit Meeting" : "Create Meeting"}</DialogTitle>
            <DialogDescription>
              {modalMode === "edit"
      ...
      <div className="text-muted-foreground text-xs">
        Showing <strong>{filteredActivities.length}</strong> of <strong>{activities.length}</strong>{" "}
        activities
      </div>
    </div>
  );
