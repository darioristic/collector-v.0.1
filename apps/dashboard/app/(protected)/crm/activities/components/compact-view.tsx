  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
      {activities.map((activity) => {
        ...
        return (
          <Card key={activity.id} className="flex flex-col border border-muted/70 shadow-none transition hover:border-primary/40">
            ...
            <CardContent className="flex flex-1 flex-col gap-3 text-sm">
              <div className="flex items-center gap-2">
                ...
                <div className="flex flex-col leading-tight">
                  ...
                  <span className="text-xs text-muted-foreground">{format(dueDate, "hh:mm a")}</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 rounded-md border bg-muted/40 p-2 text-xs">
                <Avatar className="h-8 w-8">
                  <AvatarFallback>{getAvatarFallback(activity)}</AvatarFallback>
                </Avatar>
                <div className="flex flex-col">
                  <span className="font-medium">{activity.assignedToName ?? "Unassigned"}</span>
                  <span className="text-muted-foreground">{activity.assignedToEmail ?? "No email"}</span>
                </div>
              </div>
              {activity.notes ? (
                <TooltipProvider>
              ...
              <div className="flex items-center gap-2">
                {onView ? (
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button variant="ghost" size="icon" onClick={() => onView(activity)}>
