    ...
    <div className="max-w-7xl space-y-8 p-8 mx-auto">
      ...
      <section className="grid gap-6 sm:grid-cols-2 xl:grid-cols-4">
        {KPI_METRICS.map((metric) => (
          <Card key={metric.id} className="rounded-2xl border border-[#E5E5EA] bg-white shadow-sm">
            <CardContent className="space-y-4 p-6">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <div className="mt-3 flex items-end gap-2">
                    ...
                    <Badge className={cn("rounded-full px-2.5 py-1 text-xs font-medium", trendBadgeClass[metric.trend])}>
                      {metric.change}
                    </Badge>
                  </div>
              ...
              <div className={cn("h-2 w-full rounded-full bg-gradient-to-r", metric.sparklineClass)} />
            </CardContent>
          </Card>
        ))}
      </section>
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <Card className="rounded-2xl border border-[#E5E5EA] bg-white shadow-sm">
          <CardHeader className="space-y-1">
            <CardTitle className="text-lg font-semibold text-slate-900">Leads by Platform</CardTitle>
        ...
        <Card className="rounded-2xl border border-[#E5E5EA] bg-white shadow-sm">
          <CardHeader className="space-y-1">
            <CardTitle className="text-lg font-semibold text-slate-900">Monthly Sales Performance</CardTitle>
            <CardDescription className="text-sm text-slate-500">
      ...
      <section>
        <Card className="rounded-2xl border border-[#E5E5EA] bg-white shadow-sm">
          <CardHeader className="space-y-1 border-b border-[#E5E5EA] p-6">
            ...
            <CardDescription className="text-sm text-slate-500">
              Overview of leads, deals, and conversion performance per sales rep.
            </CardDescription>
          </CardHeader>
              ...
              <Table className="min-w-[720px]">
                <TableHeader className="bg-[#F9FAFB]">
                  <TableRow>
                    ...
                    <TableHead className="text-xs uppercase tracking-wide text-slate-500">Leads</TableHead>
                    <TableHead className="text-xs uppercase tracking-wide text-slate-500">Deals Closed</TableHead>
                    <TableHead className="text-xs uppercase tracking-wide text-slate-500">
                      Conversion Rate
                    </TableHead>
                    <TableHead className="text-xs uppercase tracking-wide text-slate-500">Revenue</TableHead>
                ...
                <TableBody>
                  {SALES_SUMMARY.map((rep) => (
                    <TableRow key={rep.id} className="transition hover:bg-[#F1F5FF]">
                      <TableCell>
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs text-slate-500">Senior Account Executive</span>
                          </div>
                        </div>
                      </TableCell>
                      <TableCell className="text-slate-600">{rep.leads.toLocaleString("sr-RS")}</TableCell>
                      <TableCell className="text-slate-600">{rep.deals.toLocaleString("sr-RS")}</TableCell>
                      <TableCell className="text-slate-600">{rep.conversionRate}</TableCell>
                      <TableCell className="text-slate-600">{revenueFormatter.format(rep.revenue)}</TableCell>
                      <TableCell>
                        <Badge className={cn("rounded-full px-3 py-1 text-xs font-medium", trendBadgeClass[rep.trend])}>
                          {rep.change}
                        </Badge>
                      </TableCell>
                    </TableRow>
          ...
          <CardFooter className="flex items-center justify-between border-t border-[#E5E5EA] bg-[#FAFAFA] p-6">
            <span className="text-sm text-slate-500">
              Showing {SALES_SUMMARY.length} of {SALES_SUMMARY.length} representatives
            </span>
