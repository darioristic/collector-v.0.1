...
const PERIOD_OPTIONS: Array<{ value: FilterFormValues["period"]; label: string }> = [
...
export default function CRMOverviewPage(): JSX.Element {
  ...
  return (
    <div className="space-y-6">
      <Form {...form}>
        <form onSubmit={handleFilters} className="grid gap-6 lg:grid-cols-[300px,minmax(0,1fr)]">
            ...
            <div className="flex h-full flex-col justify-between gap-6">
              ...
              <div className="flex items-center gap-3 rounded-xl border border-[#E5E5EA] bg-[#F9FAFB] p-3">
                <Avatar className="size-10">
                  <AvatarImage src="/images/avatars/avatar-6.png" alt="Milica Nikolić" />
                  <AvatarFallback>MN</AvatarFallback>
                </Avatar>
                <div className="flex flex-1 flex-col text-sm">
                  <span className="font-semibold text-slate-700">Milica Nikolić</span>
                  <span className="text-xs text-slate-500">CRM Manager</span>
                </div>
                <Button variant="outline" size="icon" className="rounded-full border-[#E5E5EA]">
          ...
          <div className="space-y-6 rounded-2xl border border-[#E5E5EA] bg-[#FAFAFA] p-6 shadow-sm">
                        ...
                        <Popover>
                          <PopoverTrigger asChild>
                            <Button
                              ...
                              <span>{field.value ? format(field.value, "dd MMM yyyy") : "Izaberi datum"}</span>
                            </Button>
                          </PopoverTrigger>
                          <PopoverContent className="w-auto rounded-xl border-[#E5E5EA] bg-white p-0" align="end">
                            <Calendar
                              mode="single"
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">
                Pratite ključne KPI-je, interakcije i prodajne performanse vašeg tima.
              </p>
            </div>
            ...
            <div className="grid gap-6 xl:grid-cols-2">
              <Card className="rounded-2xl border-[#E5E5EA] shadow-sm">
                <CardHeader className="flex flex-col gap-1">
                  <CardTitle className="text-lg font-semibold text-slate-900">Projected Sales Growth</CardTitle>
                ...
                <CardContent className="space-y-6">
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="rounded-xl border border-[#E5E5EA] bg-[#F9FAFB] p-4">
                      <p className="text-xs uppercase tracking-wide text-slate-500">Leads Generated</p>
                      <div className="mt-2 flex items-end gap-2">
                        <span className="text-2xl font-semibold text-slate-900">1,284</span>
                        <Badge className="rounded-full bg-emerald-50 px-2 text-xs font-medium text-emerald-600">
                          +6.4%
                        </Badge>
                      </div>
                      <p className="mt-1 text-xs text-slate-500">u odnosu na prošlu nedelju</p>
                    </div>
                    <div className="rounded-xl border border-[#E5E5EA] bg-[#F9FAFB] p-4">
                      <p className="text-xs uppercase tracking-wide text-slate-500">Conversion Rate (Today)</p>
                      <div className="mt-2 flex items-end gap-2">
                        <span className="text-2xl font-semibold text-slate-900">18.7%</span>
                        <Badge className="rounded-full bg-blue-50 px-2 text-xs font-medium text-blue-600">
                          16.2% juče
                        </Badge>
                      </div>
                      <p className="mt-1 text-xs text-slate-500">trend rasta poslednjih 7 dana</p>
                    </div>
                  </div>
              ...
              <Card className="rounded-2xl border-[#E5E5EA] shadow-sm">
                ...
                <CardContent className="space-y-6">
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="rounded-xl border border-[#E5E5EA] bg-[#F9FAFB] p-4">
                      <p className="text-xs uppercase tracking-wide text-slate-500">Responses Received</p>
                      <div className="mt-2 flex items-end gap-2">
                    ...
                    <div className="rounded-xl border border-[#E5E5EA] bg-[#F9FAFB] p-4">
                      <p className="text-xs uppercase tracking-wide text-slate-500">Response Rate (Today)</p>
                      <div className="mt-2 flex items-end gap-2">
                        <span className="text-2xl font-semibold text-slate-900">87.5%</span>
                        <Badge className="rounded-full bg-blue-50 px-2 text-xs font-medium text-blue-600">
                          82.3% juče
                        </Badge>
                      </div>
                      <p className="mt-1 text-xs text-slate-500">konzistentan rast angažmana</p>
                    </div>
                  </div>
                  <div className="h-56 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={ENGAGEMENT_ACTIVITY_DATA} barGap={10} margin={{ top: 10, left: -20, right: 10, bottom: 0 }}>
                        <CartesianGrid stroke="#F3F4F6" vertical={false} />
                        <XAxis dataKey="month" axisLine={false} tickLine={false} tick={{ fill: "#6B7280", fontSize: 12 }} />
                        <YAxis axisLine={false} tickLine={false} tick={{ fill: "#6B7280", fontSize: 12 }} />
                        <Tooltip contentStyle={{ borderRadius: "12px", borderColor: "#E5E7EB" }} />
                        <Legend iconType="circle" wrapperStyle={{ fontSize: 12 }} />
                        <Bar dataKey="email" name="Email" fill="#F97316" radius={[8, 8, 0, 0]} />
                        <Bar dataKey="meeting" name="Meeting" fill="#22C55E" radius={[8, 8, 0, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </CardContent>
            ...
            <div className="rounded-2xl border border-[#E5E5EA] bg-white p-6 shadow-sm">
              <h2 className="text-lg font-semibold text-slate-900">Customer Engagement Summary</h2>
              <p className="text-sm text-slate-500">
            ...
            <div className="rounded-2xl border border-[#E5E5EA] bg-white shadow-sm">
              <div className="flex flex-col gap-4 border-b border-[#E5E5EA] p-6 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <h2 className="text-lg font-semibold text-slate-900">Customer Interactions</h2>
                  <p className="text-sm text-slate-500">
                  ...
                  <TableBody>
                    {INTERACTIONS.map((interaction) => (
                      <TableRow
                        ...
                        <TableCell className="font-medium text-slate-700">
                          <div className="flex items-center gap-3">
                            <Avatar className="size-9">
                              <AvatarImage src={interaction.avatar} alt={interaction.name} />
                              <AvatarFallback>
                                {interaction.name
                                  .split(" ")
                                  .map((part) => part[0])
                        ...
                        <TableCell className="text-slate-500">{interaction.id}</TableCell>
                        <TableCell>
                          <Badge className={cn("rounded-full px-3 py-1 text-xs font-medium", STATUS_STYLES[interaction.statusTone])}>
                            {interaction.status}
                          </Badge>
                        </TableCell>
                        <TableCell className="text-slate-500">{interaction.channel}</TableCell>
                        <TableCell className="text-slate-500">{interaction.date}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
              ...
              <div className="flex items-center justify-between border-t border-[#E5E5EA] p-6">
                <div className="text-sm text-slate-500">Prikazano {INTERACTIONS.length} interakcija</div>
                <Button variant="link" className="text-[#007AFF] hover:text-[#0050A4]">
                  Show all
                </Button>
              </div>
            </div>
