...
export default function DealsKanbanBoard({
	...
	const [columns, setColumns] =
		React.useState<Record<DealStage, Deal[]>>(groupedDeals);
		...
		(nextColumns: Record<DealStage, Deal[]>) => {
			...
			for (const stage of DEAL_STAGES) {
				const nextIds = new Set(
					nextColumns[stage]?.map((deal) => deal.id) ?? [],
				);
				...
				for (const id of prevIds) {
					if (!nextIds.has(id)) {
						...
						for (const targetStage of DEAL_STAGES) {
							...
							const targetIds =
								nextColumns[targetStage]?.map((deal) => deal.id) ?? [];
					...
					return (
						...
						>
							<div className="flex items-start justify-between gap-2 rounded-lg border border-dashed border-border/60 bg-muted/40 px-3 py-2">
								...
								<Badge className={DEAL_STAGE_BADGE_CLASSNAME[stage]}>
									{items.length}
								</Badge>
							</div>
