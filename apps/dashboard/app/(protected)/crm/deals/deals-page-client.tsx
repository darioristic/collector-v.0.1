...
import { BarChart2, Columns3, Filter, Grid as GridIcon, Loader2, Plus, Table as TableIcon } from "lucide-react";
...
import { createDeal, deleteDeal, updateDeal, updateDealStage, type DealStageSummary } from "./actions";
...
const viewOptions: Array<{ value: DealView; label: string; icon: React.ReactNode }> = [
  { value: "kanban", label: "Kanban", icon: <Columns3 className="h-4 w-4" aria-hidden="true" /> },
  { value: "table", label: "Table", icon: <TableIcon className="h-4 w-4" aria-hidden="true" /> },
  { value: "compact", label: "Compact", icon: <GridIcon className="h-4 w-4" aria-hidden="true" /> },
];
...
export default function DealsPageClient({
  ...
  const handleUpdateDeal = React.useCallback(
    ...
    [activeDeal, handleCloseModal, setSaving, startTransition, toast, updateDealInStore],
  );
