    ...
    <div className="space-y-4">
          ...
          <TableHeader className="sticky top-0 z-10 bg-card/95 backdrop-blur">
            <TableRow>
              ...
              <TableHead className="cursor-pointer" onClick={() => onSortChange("startDate")}>
                <div className="flex items-center gap-2">
                  Start date
                  <span className="text-muted-foreground text-xs">{sortedIndicator("startDate")}</span>
                </div>
              </TableHead>
          ...
          <TableBody>
            {employees.map((employee) => (
              ...
              >
                <TableCell>
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                      <span className="font-medium leading-tight">{employee.fullName}</span>
                ...
                <TableCell>
                  <Badge variant={statusVariantMap[employee.status]}>{employee.status}</Badge>
                </TableCell>
      ...
      <div className="space-y-3 lg:hidden">
        {employees.map((employee) => (
          <div
            key={employee.id}
            className="group rounded-xl border border-border bg-card p-4 shadow-sm transition hover:border-primary/40"
          >
            <div className="flex items-start justify-between gap-4">
              ...
              >
                ...
                <div className="flex flex-col">
                  <span className="font-semibold leading-tight">{employee.fullName}</span>
                  <span className="text-muted-foreground text-xs">{employee.email}</span>
      ...
      {hasNextPage ? (
        <div className="flex justify-center">
          <Button type="button" variant="outline" size="lg" onClick={onLoadMore} disabled={isFetchingNextPage}>
      ...
      {employees.length === 0 && !isLoading ? (
        <div className="text-muted-foreground text-sm text-center">No employees match your filters.</div>
      ) : null}
    </div>
  );
}
