...
export default function EmployeesToolbar({
  ...
  const hasActiveFilters =
    Boolean(filters.department) || Boolean(filters.status) || Boolean(filters.employmentType) || Boolean(searchValue);
    ...
    <div className="flex flex-col gap-4 rounded-xl border border-border bg-card/80 p-4 shadow-sm backdrop-blur md:flex-row md:items-center md:justify-between">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-end">
        <div className="flex items-center gap-2">
          <Select value={sortField} onValueChange={(value) => onSortFieldChange(value as EmployeesQueryState["sortField"])} disabled={isDisabled}>
          ...
          <Button
            ...
            className="size-10"
          >
            {sortOrder === "asc" ? <SortAsc className="size-4" aria-hidden="true" /> : <SortDesc className="size-4" aria-hidden="true" />}
          </Button>
        </div>
        ...
        <Button
          ...
          className={cn(
            ...
            hasActiveFilters ? "text-primary hover:text-primary" : "text-muted-foreground"
          )}
        >
