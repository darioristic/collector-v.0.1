...
export default function EmployeesPageClient({ initialQuery }: EmployeesPageClientProps) {
  ...
  const [formInitialValues, setFormInitialValues] = React.useState<EmployeeFormValues | undefined>(
    undefined
  );
  ...
  const handleFiltersChange = React.useCallback(
    (nextFilters: { department?: string; status?: string; employmentType?: string }) => {
      setQueryState((prev) => ({
        ...
        employmentType: isEmploymentType(nextFilters.employmentType)
  ...
  const handleTableSortChange = React.useCallback((field: EmployeesQueryState["sortField"]) => {
    setQueryState((prev) => ({
      ...prev,
      sortField: field,
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-1 text-sm">
              Stay on top of your workforce, manage status changes, and keep employee records up to
              date.
            </p>
          </div>
          <AddEmployeeDialog>
      ...
      <AddEmployeeDialog>
        <Button type="button" className="fixed right-6 bottom-6 z-50 shadow-lg sm:hidden">
