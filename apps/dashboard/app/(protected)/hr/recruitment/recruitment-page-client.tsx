...
const hasFilters = (query: CandidatesQueryState, searchValue: string) =>
	Boolean(query.status) || Boolean(query.position) || Boolean(searchValue);
...
export default function RecruitmentPageClient({
	...
	const handleFiltersChange = React.useCallback(
		(nextFilters: { status?: string; position?: string }) => {
			setQueryState((prev) => ({
				...prev,
				status: nextFilters.status as
					| CandidatesQueryState["status"]
					| undefined,
				...
				<div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
					<div>
						...
						<p className="text-muted-foreground mt-1 text-sm">
							Manage candidates and track interviews throughout the hiring
							process.
						</p>
					</div>
