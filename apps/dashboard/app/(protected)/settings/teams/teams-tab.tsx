...
export default function TeamsTab() {
	...
	const updateMutation = useMutation({
		mutationFn: (values: Parameters<typeof createTeamMember>[0]) =>
			updateTeamMember(selectedMember!.id, values),
		onSuccess: (member) => {
			toast({
				title: "Član ažuriran",
	...
	const isAllSelected =
		sortedMembers.length > 0 && selectedIds.size === sortedMembers.length;
	...
	const handleFormSubmit = async (
		values: Parameters<typeof createTeamMember>[0],
	) => {
		if (formMode === "create") {
				...
				<div className="flex flex-col items-center justify-center gap-3 rounded-xl border border-destructive/40 bg-destructive/5 p-8 text-center">
					...
					<p className="text-sm text-muted-foreground">
						{error instanceof Error
							? error.message
							: "Preuzimanje članova tima nije uspelo."}
					</p>
				</div>
			);
		}
		...
		if (sortedMembers.length === 0) {
			return (
				<div className="flex flex-col items-center justify-center gap-3 rounded-xl border border-dashed border-muted p-10 text-center">
					...
					<p className="text-sm text-muted-foreground">
						Podesite kriterijume ili dodajte novog člana kako biste popunili ovu
						listu.
					</p>
								...
								<TableRow key={member.id} className="border-border">
									<TableCell>
										<Checkbox
											...
											onCheckedChange={(checked) =>
												toggleMemberSelection(member.id, Boolean(checked))
											}
										/>
									</TableCell>
									<TableCell>
	...
	return (
		<div className="flex flex-col gap-6">
					...
					<div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
						...
						<div className="flex flex-wrap items-center gap-3">
							...
							{isFetching && !isLoading ? (
								<Badge variant="secondary">Osvežavanje...</Badge>
							) : null}
						</div>
					</div>
			...
			<AlertDialog
				...
				onOpenChange={(open) => (!open ? setMemberToDelete(null) : null)}
			>
				<AlertDialogContent>
					<AlertDialogHeader>
						<AlertDialogTitle>Obriši člana tima</AlertDialogTitle>
						<AlertDialogDescription>
							Da li ste sigurni da želite da uklonite člana{" "}
							{memberToDelete?.fullName}? Ova radnja je trajna.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
