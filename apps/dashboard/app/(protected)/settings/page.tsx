...
const NOTIFICATION_CHANNELS: NotificationChannel[] = [
  ...
  },
  {
    key: "integrations",
    label: "Integrations",
    description: "Receive messages inside your preferred chat tools such as Slack or Discord.",
    icon: Plug2,
    hasAction: true
  },
  {
    key: "mobile",
    label: "Mobile",
    description: "Get notifications directly to your mobile inbox on iOS or Android.",
    icon: Smartphone
  },
  {
    key: "desktop",
    label: "Desktop",
    description: "Receive notifications on the desktop app or browser when you’re signed in.",
    icon: Monitor
  }
];
...
export default function SettingsPage() {
  ...
  const [channelPreferences, setChannelPreferences] = useState<Record<string, boolean>>(() =>
    NOTIFICATION_CHANNELS.reduce<Record<string, boolean>>((acc, channel) => {
      acc[channel.key] = ["inbox", "email", "integrations"].includes(channel.key);
      return acc;
    }, {})
  );
  ...
  >(() =>
    PROJECT_NOTIFICATIONS.reduce<Record<string, { email: boolean; push: boolean }>>(
      (acc, notification) => {
  ...
  const onSubmit = form.handleSubmit(async (values) => {
    try {
      ...
      const message = error instanceof Error ? error.message : "Čuvanje nije uspelo.";
      toast({
        title: "Greška",
        description: message,
        variant: "destructive"
