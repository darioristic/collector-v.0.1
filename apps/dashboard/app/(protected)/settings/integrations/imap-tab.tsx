...
async function apiConnect(): Promise<{
	ok: boolean;
...
async function apiGetSmtpStatus(): Promise<SmtpStatus> {
	const res = await fetch("/api/integrations/smtp/status", {
		cache: "no-store",
	});
	const data = await res.json().catch(() => null);
...
async function apiSaveSmtpSettings(
...
async function apiSendTestEmail(
	to: string,
			...
			<div className="flex flex-col gap-6 rounded-2xl border border-border bg-card p-6 md:p-8">
				<div className="flex flex-col gap-2">
					...
					<p className="text-sm text-muted-foreground">
						Configure your IMAP connection for syncing emails, contacts and
						calendar.
					</p>
				</div>
				...
				<div className="grid gap-4 md:grid-cols-2">
					...
					<div className="flex items-center gap-3">
						<Switch
							...
							onCheckedChange={(v) => setSettings((s) => ({ ...s, secure: v }))}
						/>
				...
				<div className="grid gap-4 md:grid-cols-3">
					...
					<div className="flex items-center gap-3">
						<Switch
							...
							onCheckedChange={(v) =>
								setSettings((s) => ({ ...s, syncContacts: v }))
							}
						/>
					...
					<div className="flex items-center gap-3">
						<Switch
							...
							onCheckedChange={(v) =>
								setSettings((s) => ({ ...s, syncCalendar: v }))
							}
						/>
						...
						onClick={() => {
							if (!settings.host || !settings.username || !settings.password) {
								toast({
									...
									description:
										"Please fill in all required fields (host, username, password)",
									variant: "destructive",
								});
								return;
							}
				...
				<div className="text-sm text-muted-foreground">
					...
					{statusData?.message && (
						<div
							className={`mt-1 ${statusData.connected ? "text-green-600" : "text-red-600"}`}
						>
							{statusData.message}
						</div>
					)}
				...
				<div className="grid gap-4 md:grid-cols-2">
					...
					<div className="flex flex-col gap-2">
						...
						<Input
							...
							onChange={(e) =>
								setSmtpSettings((s) => ({ ...s, username: e.target.value }))
							}
						/>
					</div>
					<div className="flex flex-col gap-2">
						...
						<Input
							...
							onChange={(e) =>
								setSmtpSettings((s) => ({ ...s, password: e.target.value }))
							}
						/>
					</div>
					<div className="flex flex-col gap-2 md:col-span-2">
						...
						<Input
							...
							onChange={(e) =>
								setSmtpSettings((s) => ({ ...s, fromEmail: e.target.value }))
							}
						/>
					</div>
				</div>
				...
				<div className="flex flex-wrap gap-3">
						...
						onClick={() => {
							if (
								...
								toast({
									title: "Validation error",
									description:
										"Please fill in all required fields (host, username, password, from email)",
									variant: "destructive",
								});
								return;
							}
					...
					<div className="flex items-center gap-3">
						...
						<Button
							variant="secondary"
							onClick={() => sendTestMutation.mutate(smtpTestTo)}
							disabled={sendTestMutation.isPending || !smtpTestTo}
						>
							Send test
						</Button>
					</div>
				...
				<div className="text-sm text-muted-foreground">
					...
					{smtpStatus?.message && (
						<div
							className={`mt-1 ${smtpStatus.connected ? "text-green-600" : "text-red-600"}`}
						>
							{smtpStatus.message}
						</div>
					)}
