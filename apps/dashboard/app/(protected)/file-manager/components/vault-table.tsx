...
export function VaultTable() {
  const { currentFolder, currentPath, breadcrumbs, navigateToPath, createFolder, getItemSubtitle } =
    useFileManager();
  ...
  const handleCreateFolder = () => {
    ...
    if (!result.success) {
      setCreateError(result.error ?? "An unexpected error occurred while creating the folder.");
      return;
    }
                      ...
                      <div className="flex items-center gap-3">
                        <Avatar className="size-8">
                          {item.owner.avatarUrl ? (
                            ...
                            <AvatarFallback>
                              {getOwnerInitials(item.owner.name, item.owner.email)}
                            </AvatarFallback>
                          )}
                        </Avatar>
