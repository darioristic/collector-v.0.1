...
const isSameOwner = (a: FileSystemOwner, b: FileSystemOwner) =>
  a.id === b.id && a.name === b.name && a.email === b.email && a.avatarUrl === b.avatarUrl;
...
const resolveOwnerFromTeamMembers = (
  ...
  const member = teamMembers[((normalizedIndex % teamMembers.length) + teamMembers.length) % teamMembers.length];
...
const mapNodeOwners = (node: FileSystemNode, teamMembers: TeamMember[]): FileSystemNode => {
  const nextOwner = resolveOwnerFromTeamMembers(node.ownerIndex, teamMembers, node.ownerFallback);
  const ownerChanged = !isSameOwner(node.owner, nextOwner);

  if (node.type === "folder") {
    const nextChildren = node.children.map((child) => mapNodeOwners(child, teamMembers));
...
const updateOwnersWithTeamMembers = (root: FolderNode, teamMembers: TeamMember[]): FolderNode => {
  const nextOwner = resolveOwnerFromTeamMembers(root.ownerIndex, teamMembers, root.ownerFallback);
  const ownerChanged = !isSameOwner(root.owner, nextOwner);
  const nextChildren = root.children.map((child) => mapNodeOwners(child, teamMembers));
  const childrenChanged = nextChildren.some((child, index) => child !== root.children[index]);
...
const createFolderNode = (name: string, teamMembers: TeamMember[]): FolderNode => {
  const now = new Date();
...
const addFolderToPath = (folder: FolderNode, path: string[], newFolder: FolderNode): FolderNode => {
  if (path.length === 0) {
    return {
      ...folder,
...
export function FileManagerProvider({ children }: { children: ReactNode }) {
  ...
  const resolved = useMemo(() => resolvePath(fileSystem, currentPath), [fileSystem, currentPath]);
  ...
  useEffect(() => {
    ...
    const loadTeamMembers = async () => {
      try {
        ...
        console.error("Failed to load team members for file manager", error);
        if (isMounted) {
          setTeamMembers([]);
        }
      } finally {
  ...
  useEffect(() => {
    if (sanitizedPath.length !== currentPath.length || sanitizedPath.some((segment, index) => segment !== currentPath[index])) {
  ...
  const rootFolders = useMemo(
    () =>
      fileSystem.children.filter((child): child is FolderNode => child.type === "folder"),
    [fileSystem.children]
  );
  ...
  const createFolder = useCallback(
    (name: string) => {
      ...
      if (exists) {
        return { success: false, error: "A folder with that name already exists in this directory." };
      }

      const newFolder = createFolderNode(trimmedName, teamMembers);
      setFileSystem((prev) => updateOwnersWithTeamMembers(addFolderToPath(prev, target.validPath, newFolder), teamMembers));
...
export function useFileManager() {
  ...
  if (!context) {
    throw new Error("useFileManager must be used within a FileManagerProvider component.");
  }
