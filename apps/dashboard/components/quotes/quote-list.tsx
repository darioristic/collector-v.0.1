...
export function QuoteList({
	...
	const [sortField, setSortField] =
		useState<QuoteSortField>(DEFAULT_SORT_FIELD);
	...
	const quotesNeedingCompany = quotes.filter(
		(q) => q.companyId && !q.companyName,
	);
	const companyIds = Array.from(
		new Set(
			quotesNeedingCompany
				.map((q) => q.companyId)
	...
	const handleSelectAll = (checked: boolean) => {
		if (checked) {
			setSelectedQuotes(
				new Set(quotesWithCompanyNames.map((quote) => quote.id)),
			);
		} else {
	...
	const handleBulkDelete = async () => {
		if (
			!confirm(
				`Are you sure you want to delete ${selectedQuotes.size} quote(s)?`,
			)
		) {
			return;
		}
		...
		try {
			await Promise.all(
				Array.from(selectedQuotes).map((id) => deleteQuote.mutateAsync(id)),
			);
			setSelectedQuotes(new Set());
		} catch (error) {
			...
			<TableToolbar
				...
				filters={
					<Select
						...
						onValueChange={(value) => {
							setStatusFilter(
								value === "__all__" ? undefined : (value as QuoteStatus),
							);
							setPage(0);
						}}
						disabled={isFetching && !quotes.length}
					...
					<div className="flex h-96 items-center justify-center p-8">
						<Empty>
							<EmptyHeader>
								...
								<EmptyDescription>
									Create your first quote to get started with managing your
									sales quotes.
								</EmptyDescription>
							</EmptyHeader>
							<EmptyContent>
							...
							<div className="text-muted-foreground flex flex-col gap-3 text-sm md:flex-row md:items-center md:justify-end">
								...
								<div className="flex items-center gap-2">
									<span>
										...
										{Math.min((page + 1) * limit, total)} of {total} quotes
									</span>
								</div>
								{totalPages > 1 && (
									<>
