...
export function TableToolbar({
  ...
  const renderSort = () => {
    if (!sort) return null;
    const orderLabel = sort.order === "asc" ? sort.descLabel ?? "Sort descending" : sort.ascLabel ?? "Sort ascending";
    ...
    return (
      <div className="flex items-center gap-2">
        ...
        >
          <SelectTrigger className="md:w-44" aria-label={sort.ariaLabel ?? "Sort table"}>
        ...
        {reset && !(reset.hideUntilActive && resetDisabled) ? (
          <Button
            ...
            className={cn(
              ...
              !resetDisabled ? "text-primary hover:text-primary" : "text-muted-foreground"
            )}
          >
