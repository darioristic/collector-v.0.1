...
function SidebarProvider({
	...
	// This ensures server and client always use the same initial value
	const [frozenDefaultOpen] = React.useState<boolean>(() => defaultOpen);

	// This is the internal state of the sidebar.
	// We use openProp and setOpenProp for control from outside the component.
	const [_open, _setOpen] = React.useState(frozenDefaultOpen);
	const [isMounted, setIsMounted] = React.useState(false);
	const open = openProp ?? _open;

	// Set isMounted to true after component mounts to prevent hydration mismatch
	// Use useEffect to ensure it runs after hydration is complete
	// This ensures server and client render the same initial state
	React.useEffect(() => {
		setIsMounted(true);
	}, []);

	// Sync with cookie on client side after mount to prevent hydration mismatch
	// Only read cookie after isMounted is true to ensure consistent initial render
	React.useEffect(() => {
		if (isMounted && !openProp && typeof document !== "undefined") {
	...
	// For hydration safety, we freeze state based on frozenDefaultOpen to ensure server and client render the same
	const [frozenState] = React.useState<"expanded" | "collapsed">(() => {
...
function Sidebar({
	...
	// Freeze data-collapsible value based on frozenDefaultOpen to prevent hydration mismatch
	// This ensures server and client always render the same value
	const [dataCollapsible] = React.useState<string | undefined>(() => {
		return frozenDefaultOpen ? undefined : collapsible;
	});

	// Always render desktop version on server and first client render to prevent hydration mismatch
	// Then switch to mobile version if needed after hydration
	const [shouldRenderMobile, setShouldRenderMobile] = React.useState(false);
	...
	// Render mobile version only after hydration if isMobile is true
