...
const Toolbar = ({ editor }: { editor: Editor }) => {
	...
	const runAI = React.useCallback(
		async (
			action: "summarize" | "improve" | "rewrite" | "extractTasks",
			...
			<div className="flex w-max items-center gap-px">
				...
				<DropdownMenu>
					<DropdownMenuTrigger asChild>
						<ToolbarButton
							tooltip={isLoading ? "Generating..." : "AI actions"}
					...
					<DropdownMenuContent align="start" className="min-w-64">
						<DropdownMenuItem onClick={() => runAI("summarize")}>
							Summarize
						</DropdownMenuItem>
						...
						<DropdownMenuItem onClick={() => runAI("extractTasks")}>
							Extract tasks
						</DropdownMenuItem>
						<DropdownMenuSeparator />
						<DropdownMenuSub>
							...
							<DropdownMenuSubContent>
								<DropdownMenuItem
									onClick={() =>
										runAI(
											"improve",
											"Format as concise client notes with bullet points for action items, decisions, and follow-ups.",
										)
									}
								>
									Client notes
								...
								<DropdownMenuItem
									onClick={() =>
										runAI(
											"improve",
											"Produce a clear task description including objectives, acceptance criteria, and dependencies.",
										)
									}
								>
									Task description
								...
								<DropdownMenuItem
									onClick={() =>
										runAI(
											"improve",
											"Summarize customer profile including background, preferences, recent interactions, and next steps.",
										)
									}
								>
									Customer profile
						...
						<div className="p-2">
							...
							<div className="mt-2">
								<DropdownMenuItem
									onClick={() => runAI("improve", instructions)}
									disabled={isLoading || !editor.getText()}
								>
									Apply with instructions
								</DropdownMenuItem>
							</div>
						</div>
					</DropdownMenuContent>
