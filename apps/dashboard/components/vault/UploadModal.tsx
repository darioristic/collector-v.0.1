...
export function UploadModal({
	...
	const rootOption = useMemo(
		() => folders.find((item) => item.id === null),
		[folders],
	);
	...
	const folderOptions = useMemo(() => {
		...
		return folders
			.filter((folder): folder is VaultDirectoryOption & { id: string } =>
				Boolean(folder.id),
			)
			.map((folder) => ({
				id: folder.id,
		...
		<Dialog open={isOpen} onOpenChange={handleOpenChange}>
			<DialogContent className="max-w-xl">
				<DialogHeader>
					...
					<DialogDescription>
						Dodajte nove dokumente u odabrani folder.
					</DialogDescription>
				</DialogHeader>
				...
				<DialogFooter className="gap-2">
					...
					<Button
						...
						disabled={
							isSubmitting || state.files.length === 0 || !selectedFolderId
						}
					>
