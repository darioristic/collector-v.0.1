...
export function InvoiceEditor({ invoice, onCancel, onSaved }: Props) {
	...
	const stringToDoc = React.useCallback(
		(text: string) => ({
			type: "doc",
			content: (text ?? "").split("\n").map((line) => ({
				type: "paragraph",
	...
	const updateLineItem = (
		idx: number,
		patch: Partial<EditorState["lineItems"][number]>,
	) => {
		setState((prev) => {
				...
				<div className="grid grid-cols-1 gap-4 md:grid-cols-3">
					...
					<div className="space-y-2">
						...
						<Input
							...
							onChange={(e) =>
								setState((s) => ({ ...s, issuedAt: e.target.value || null }))
							}
						/>
					</div>
					<div className="space-y-2">
				...
				<div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-3">
					<div className="space-y-2">
						...
						<Input
							...
							onChange={(e) =>
								setState((s) => ({ ...s, currency: e.target.value || null }))
							}
						/>
					</div>
					<div className="space-y-2">
						...
						<Input
							...
							onChange={(e) =>
								setState((s) => ({
									...s,
									customerName: e.target.value || null,
								}))
							}
						/>
					</div>
					...
					<div>
						...
						<MinimalTiptapEditor
							...
							onChange={(val) =>
								setState((s) => ({
									...s,
									paymentDetails: contentToString(val),
								}))
							}
							output="json"
							editorContentClassName="min-h-24 font-mono text-[11px] leading-4"
