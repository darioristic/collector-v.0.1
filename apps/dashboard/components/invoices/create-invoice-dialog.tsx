...
import { Building2, FileText, Globe, Mail, MapPin, Phone, Plus, Package, Loader2, Search, Pencil } from "lucide-react";
...
function parsePastedDescription(text: string): string {
	...
	const titleLine = contentLines.find(line => 
		line.length <= 80 && 
		line.length >= 3 && 
		!/^(SLA|End Customer|Contract|Validity|Service|Account|User|TBD|SUSE|Linux|Enterprise|Server|Application)/i.test(line) &&
	...
	const productLines = contentLines.filter(line => {
		const hasTechnicalTerms = /(Server|Application|Enterprise|Support|Service|Priority|Virtual|Sockets|x86|Linux|SAP)/i.test(line);
	...
	if (productLines.length > 0) {
		...
		if (mainProductMatch && mainProductMatch[1].trim().length > 10 && mainProductMatch[1].trim().length < 100) {
			return mainProductMatch[1].trim();
		}
		
		// If no good match, use first 80 characters of the longest line
		return longestProductLine.length > 80 ? longestProductLine.substring(0, 77) + "..." : longestProductLine;
	}
		...
		<div ref={containerRef} className="relative w-full">
			<div className="relative">
				...
				<Input
					...
					className="h-8 pl-9 pr-3 text-sm border-border/60 bg-background"
					autoComplete="off"
				/>
			</div>
			{isOpen && (
				<div className="absolute z-50 mt-1.5 left-0 rounded-lg border border-border/80 bg-popover shadow-2xl backdrop-blur-md" style={{ width: "68%", minWidth: "550px", maxWidth: "calc(100vw - 4rem)" }}>
					{isLoading ? (
						<div className="flex items-center justify-center px-4 py-8">
							...
							<span className="ml-2.5 text-sm text-muted-foreground">Searching...</span>
						</div>
					) : hasResults ? (
							...
							{sortedProducts.map((product, idx) => {
								...
								return (
									<button
										...
										className={cn(
											"w-full rounded-lg px-4 py-3 text-left transition-all duration-150 mx-1",
											"hover:bg-accent hover:shadow-sm",
											"focus:bg-accent focus:outline-none focus:ring-2 focus:ring-primary/20",
											selectedIndex === idx && "bg-accent shadow-sm ring-2 ring-primary/20",
										)}
						...
						<div className="px-3 py-3">
							<button
								...
								className="w-full rounded-lg border-2 border-dashed border-primary/40 bg-gradient-to-r from-primary/5 to-primary/10 p-4 text-left transition-all duration-200 hover:border-primary/60 hover:from-primary/10 hover:to-primary/15 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-primary/30 active:from-primary/15 active:to-primary/20"
							>
								<div className="flex items-start gap-3">
									...
									<div className="flex flex-1 flex-col gap-1.5 min-w-0 overflow-hidden">
										<div className="font-bold text-sm leading-tight text-primary line-clamp-2 break-words">
											Create "{trimmedQuery}"
										</div>
						...
						<div className="px-4 py-6 text-center">
							<div className="flex flex-col items-center gap-2">
								...
								<p className="text-sm font-medium text-muted-foreground">Type at least 3 characters</p>
								<p className="text-xs text-muted-foreground/70">to search for products</p>
							</div>
						</div>
					) : (
						<div className="px-4 py-6 text-center">
...
export function CreateInvoiceDialog({
	...
	const getDefaultCurrency = () => {
		...
		if (browserLocale.toLowerCase().includes("rs") || 
	...
	const primaryAddress = customerData?.addresses?.find(
		(addr) => addr.label === "primary" || addr.label === "billing",
	...
	const totals = useMemo(() => {
		...
		const amountBeforeDiscount = currentItems.reduce(
			(sum, item) => {
		...
		const discountTotal = currentItems.reduce(
			(sum, item) => {
				const qty = Number(item?.quantity) || 0;
				const price = Number(item?.unitPrice) || 0;
		...
		const totalVat = currentItems.reduce(
			(sum, item) => {
				const qty = Number(item?.quantity) || 0;
				const price = Number(item?.unitPrice) || 0;
				const discountRate = Number(item?.discountRate) || 0;
		...
		const itemsWithSubtotal = currentItems.filter(
			(item) => {
				const qty = Number(item?.quantity) || 0;
				const price = Number(item?.unitPrice) || 0;
				const vatRate = Number(item?.vatRate) || 0;
			...
			const productCreationPromises = validItems
				...
				.map(async (item) => {
					try {
						...
						const exists = existingProducts.some((p) => p.name.toLowerCase() === item.description!.toLowerCase().trim());
	...
	const handleAddItem = () => {
		...
		<Sheet open={open} onOpenChange={onOpenChange}>
			<SheetContent
				side="right"
				className="w-full p-0 md:w-[calc(50vw)] md:max-w-[900px] overflow-y-auto [&>button]:hidden"
			>
				<VisuallyHidden>
										...
										<div className="border-border/60 bg-muted/40 rounded-lg border p-3">
											<div className="mb-4 space-y-4 pb-4">
												<div className="flex items-center justify-between gap-3">
													...
													<Badge 
														variant={getStatusBadgeVariant(selectedStatus) as any}
												...
												<div className="flex flex-wrap items-center gap-2 text-muted-foreground text-sm">
													<Popover
														open={issuedDateOpen}
														onOpenChange={setIssuedDateOpen}
													>
														...
														{!selectedCustomerId ? (
															...
															<div className="space-y-3 text-sm relative">
																<Button
																	...
																	variant="ghost"
																	size="icon"
																	className="absolute -top-2 right-0 h-8 w-8 text-muted-foreground hover:text-foreground hover:bg-muted"
																...
																<div>
																	<p className="text-foreground text-base font-semibold pr-10">
																		{customer.name}
																	</p>
																	{(() => {
																		...
																		const hasPostalCode = addr.postalCode && addr.postalCode.trim();
																...
																<div className="flex flex-wrap items-center gap-4 pt-2 border-t">
																	...
																	{(() => {
																		const companyId = customer?.registrationNumber || customerData?.account?.registrationNumber;
																...
																<TableRow key={field.id} className="hover:bg-muted/50">
																	<TableCell className="px-3 py-2 text-sm">
																		...
																		onProductSelect={(product) => {
																			...
																			if (price > 0) {
																				methods.setValue(`items.${index}.unitPrice`, price, { shouldValidate: true, shouldDirty: true });
																			}
																			
																			const description = product.description || product.name;
																			if (description) {
																				methods.setValue(`items.${index}.description`, description, { shouldValidate: true, shouldDirty: true });
																			}
																		}}
																		/>
																	</TableCell>
											...
											const isRSD = selectedCurrency === "RSD";
																						...
																						if (isRSD) {
																							...
																							if (lastCommaIndex > -1) {
																								...
																								const beforeComma = value.substring(0, lastCommaIndex).replace(/\./g, "");
																								const afterComma = value.substring(lastCommaIndex + 1).replace(/,/g, "");
																								// Add thousands separators
																								const formatted = beforeComma.replace(/\B(?=(\d{3})+(?!\d))/g, ".");
																								formattedValue = formatted + "," + afterComma;
																								// Parse for setValue
																								numValue = Number.parseFloat(beforeComma.replace(/\./g, "") + "." + afterComma) || 0;
																							} else {
																							...
																							if (lastDotIndex > -1) {
																								...
																								const beforeDot = value.substring(0, lastDotIndex).replace(/,/g, "");
																								const afterDot = value.substring(lastDotIndex + 1).replace(/\./g, "");
																								// Add thousands separators
																								const formatted = beforeDot.replace(/\B(?=(\d{3})+(?!\d))/g, ",");
																								formattedValue = formatted + "." + afterDot;
																								// Parse for setValue
																								numValue = Number.parseFloat(beforeDot + "." + afterDot) || 0;
																							} else {
																						...
																						methods.setValue(`items.${index}.unitPrice` as const, numValue, {
																			...
																			className="h-8 w-full max-w-[120px] text-sm text-right"
																		/>
																	</TableCell>
																	<TableCell className="px-3 py-2 text-sm text-right">
																		<Input
																			...
																			{...methods.register(
																				`items.${index}.discountRate` as const,
																				{
																					...
																					onChange: (e) => {
																						...
																						// Update form value immediately for real-time calculations
																						methods.setValue(`items.${index}.discountRate` as const, numValue, {
																							shouldValidate: false,
																							shouldDirty: true,
																						});
																					},
																					onBlur: (e) => {
																						...
																						e.target.value = selectedCurrency === "RSD" 
																							? formatted.replace(".", ",") 
																							: formatted;
																					},
																				},
																			...
																			{...methods.register(
																				`items.${index}.vatRate` as const,
																				{
																					...
																					onChange: (e) => {
																						...
																						// Update form value immediately for real-time calculations
																						methods.setValue(`items.${index}.vatRate` as const, numValue, {
																							shouldValidate: false,
																							shouldDirty: true,
																						});
																					},
													...
													<TableFooter>
														<TableRow>
															<TableCell colSpan={6} className="px-3 py-2 text-sm text-right font-medium">
														...
														<TableRow>
															<TableCell colSpan={6} className="px-3 py-2 text-sm text-right font-medium">
																VAT Amount{totals.averageVatRate > 0 ? ` (${totals.averageVatRate}%)` : ""}:
															</TableCell>
															<TableCell className="px-3 py-2 text-sm text-right font-medium">
														...
														<TableRow className="border-t-2">
															<TableCell
																colSpan={6}
																className="px-3 py-2.5 text-base font-bold text-right"
															>
																Total:
															</TableCell>
															<TableCell className="px-3 py-2.5 text-base font-bold text-right">
