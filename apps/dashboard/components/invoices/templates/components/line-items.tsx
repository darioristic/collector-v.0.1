...
export function LineItems({
	...
	const gridCols = includeVAT
		? "grid-cols-[24px_2fr_8%_8%_10%_8%_8%_80px]"
		: "grid-cols-[24px_2fr_10%_10%_12%_80px]";
	...
	const itemsIds = React.useMemo(
		() =>
			lineItems.map((it, i) =>
				it.id ? `line-item-${it.id}` : `line-item-${i}`,
			),
		[lineItems],
	);

	const onDescriptionKeyDown = React.useCallback(
	...
	const gridStatic = includeVAT
		? "24px 2fr 56px 60px 80px 60px 60px 100px"
		: "24px 2fr 56px 60px 80px 100px";
							...
							return (
								...
								>
									...
									>
										...
										<div className="self-start text-center text-[11px]">
											<input
												...
												<div className="self-start text-center text-[11px]">
													<input
														...
														onBlur={(e) => {
															...
															onChangeDiscount?.(
																index,
																parseNumber(v, item.discountRate ?? 0),
															);
															setDiscountInput((p) => {
																const next = { ...p };
																delete next[index];
				...
				lineItems.map((item, index) => {
					...
					return (
						...
						>
							...
							<div className="self-start text-center text-[11px] tabular-nums font-medium">
								{formatNumber(itemTotal)}
							</div>
						</div>
					);
...
const SortableHandleCtx = React.createContext<SortableHandleCtxType | null>(
	null,
);
