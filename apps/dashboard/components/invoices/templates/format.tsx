    ...
    <>
      {doc.content.map((node, nodeIndex) => {
        if (node.type === "paragraph") {
          ...
          const processText = (text: string, marks?: EditorMark[]): React.ReactNode[] => {
            ...
            <p key={`paragraph-${nodeIndex.toString()}`} className="mb-1">
              {node.content?.map((inlineContent, inlineIndex) => {
                if (inlineContent.type === "text") {
                  return (
                    <React.Fragment key={`fragment-${nodeIndex}-${inlineIndex}`}>
                      {processText(inlineContent.text || "", inlineContent.marks)}
                    </React.Fragment>
                  );
                }
