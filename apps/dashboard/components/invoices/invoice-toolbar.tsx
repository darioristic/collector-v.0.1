...
export function InvoiceToolbar({
	...
	const recenter = React.useCallback(() => {
		...
		const anchor = currentAnchorId
			? document.getElementById(currentAnchorId)
			: null;
	...
	React.useEffect(() => {
		...
		const imgs = anchor
			? Array.from(anchor.querySelectorAll<HTMLImageElement>("img"))
			: [];
	...
	return (
		...
		>
			<div
				className={`pointer-events-auto flex items-center justify-center gap-1 bg-transparent rounded-full px-2 py-2 h-9 transition-all duration-300 ${pulse ? "" : ""}`}
			>
				<div className="relative group">
					...
					<span className="absolute -top-5 left-1/2 -translate-x-1/2 text-[10px] font-medium opacity-0 group-hover:opacity-100">
						Download
					</span>
				</div>
