...
export function InvoiceDetail({
	...
	const [activeAutocompleteIndex, setActiveAutocompleteIndex] = React.useState<
		number | undefined
	>(undefined);
	...
	const mapInvoiceToTemplateProps = React.useMemo((): TemplateProps | null => {
		...
		const defaultTemplate: TemplateConfig = {
			logo_url: company?.logoUrl ?? "/logo.png",
			from_label: "From",
			customer_label: "To",
		...
		const lineItems: LineItem[] = (invoice.items || []).map(
			(item: InvoiceItem) => ({
		...
		const createEditorContent = (
			text: string,
		): import("./templates/types").EditorDoc => {
			if (!text) {
		...
		const fromDetails = createEditorContent(
			`${company?.name ?? "Your Company"}\n${company?.email ?? "info@yourcompany.com"}\n${company?.phone ?? "+381 60 000 0000"}\n${company?.streetAddress ?? "Address line"}\nVAT ID: ${company?.taxId ?? "—"}`,
		);
	...
	React.useEffect(() => {
		...
		setEditNotes(
			typeof invoice.notes === "string" ? (invoice.notes as string) : "",
		);
		...
		setEditLineDiscount(
			(invoice.items || []).map((it) => it.discountRate || 0),
		);
	}, [invoice]);
	...
	const handleSaveInline = async () => {
		...
		try {
			...
			const parsedCustomerName =
				customerLines[0]?.trim() || invoice.customerName;
			const parsedCustomerEmail =
				customerLines[1]?.trim() ||
				(invoice.customerEmail as string | undefined);
			const parsedBillingAddress =
				customerLines[2]?.trim() ||
				(invoice.billingAddress as string | undefined);
			...
			const payload: import("@crm/types").InvoiceUpdateInput = {
				...
				items: (() => {
					...
					const items = [] as Array<
						import("@crm/types").InvoiceItemCreateInput
					>;
	...
	const handleLineItemDescriptionChange = React.useCallback(
	...
	React.useEffect(() => {
		...
		const timer = setTimeout(async () => {
			...
			for (let i = 0; i < maxLen; i++) {
				...
				if (
					(pendingItems[i] !== undefined && pendingItems[i] !== last) ||
					current !== last
				) {
			...
			try {
				...
				await updateInvoice({
					...
					input: {
						items: fullItems,
					} as import("@crm/types").InvoiceUpdateInput,
				});
									...
									onClick={async () => {
										try {
											...
											const response = await fetch(
												`/api/sales/invoices/${invoiceId}/send`,
												{
													method: "POST",
													headers: {
											...
											if (!response.ok) {
												...
												throw new Error(
													error.error || "Failed to send invoice",
												);
											}
							...
							<HtmlTemplate
								...
								editors={{
									from: {
										value: editFrom,
										onChange: (v: unknown) =>
											setEditFrom(typeof v === "string" ? v : ""),
									},
									customer: {
										value: editCustomer,
										onChange: (v: unknown) =>
											setEditCustomer(typeof v === "string" ? v : ""),
									},
									payment: {
										value: editPayment,
										onChange: (v: unknown) =>
											setEditPayment(typeof v === "string" ? v : ""),
									},
									notes: {
