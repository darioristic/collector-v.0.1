...
export function ProjectTimeline({
	...
	const [editingEvent, setEditingEvent] = useState<ProjectTimelineEvent | null>(
		null,
	);
			...
			toast({
				...
				description:
					error instanceof Error
						? error.message
						: "Došlo je do greške pri čuvanju vremenske linije.",
			});
		}
	};
		...
		try {
			...
			toast({
				...
				description:
					error instanceof Error
						? error.message
						: "Došlo je do greške pri brisanju stavke.",
			});
		}
	};
		...
		<Card className="border-none bg-card/80 shadow-lg shadow-primary/5">
			<CardHeader className="flex flex-row items-start justify-between gap-4">
				<div>
					...
					<p className="text-muted-foreground text-sm">
						Vizuelni prikaz ključnih momenata projekta sa statusima u realnom
						vremenu.
					</p>
				</div>
			...
			<CardContent className="pt-2">
				{sortedEvents.length === 0 ? (
					<div className="rounded-lg border border-dashed p-8 text-center text-sm text-muted-foreground">
						Još uvek nema definisanih koraka. Dodajte prvi milestone kako biste
						pratili napredak.
					</div>
				) : (
			...
			<Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
				<DialogContent className="sm:max-w-md">
					<DialogHeader>
						<DialogTitle>
							{editingEvent ? "Ažuriraj milestone" : "Dodaj novi milestone"}
						</DialogTitle>
					</DialogHeader>
