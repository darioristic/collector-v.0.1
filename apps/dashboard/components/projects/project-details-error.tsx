...
> {
  ...
  static getDerivedStateFromError(error: Error): ProjectDetailsErrorBoundaryState {
    return { error };
  }
...
export function ProjectDetailsNotFoundState({
  ...
  return (
    <ProjectStatusShell
      ...
      actions={
        <Fragment>
          <Button onClick={() => router.push("/project-list")} className="gap-2">
...
function ProjectDetailsErrorState({ error, projectId, onRetry }: ProjectDetailsErrorStateProps) {
  const router = useRouter();
  ...
  const showRetry = variant !== "not-found" || error.message.toLowerCase().includes("timeout");
    ...
    >
      ...
      <div className="space-y-3">
        <div className="space-y-1">
          <p className="font-display text-2xl font-semibold tracking-tight lg:text-3xl">{title}</p>
        ...
        <details className="group w-full rounded-xl border border-border/60 bg-muted/50 p-4 text-left text-sm text-muted-foreground">
          ...
          <div className="mt-3 rounded-md border border-border/50 bg-background/80 px-3 py-2">
            <p className="text-xs text-muted-foreground/90">
              Ako problem potraje, podeli ove informacije sa timom koji održava aplikaciju.
            </p>
          </div>
        </details>
      ) : null}
      <div className="text-xs text-muted-foreground/70">
        ...
        <Link href="/project-list" className="font-medium text-foreground underline underline-offset-4">
...
function resolveErrorVariant(error: Error): "offline" | "not-found" | "generic" {
