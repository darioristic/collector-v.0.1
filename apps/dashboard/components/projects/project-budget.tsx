...
export function ProjectBudget({
	...
	const [editingCategory, setEditingCategory] =
		useState<ProjectBudgetCategory | null>(null);
	...
	const utilisation = useMemo(() => {
		...
		return {
			...
			remaining,
			percent:
				total === 0 ? 0 : Math.min(Math.round((spent / total) * 100), 100),
		};
	}, [budget]);
	...
	const chartData = [
		{
			name: "Potrošeno",
			value: utilisation.spent,
			fill: "hsl(var(--primary))",
		},
		{
			name: "Preostalo",
			value: utilisation.remaining,
			fill: "hsl(var(--muted))",
		},
	];
	...
	return (
		<Card className="bg-card/80 shadow-primary/5 border-none shadow-lg">
			<CardHeader className="flex flex-row items-start justify-between gap-4">
				<div className="space-y-1">
					<CardTitle className="text-xl font-semibold">Budžet</CardTitle>
					<p className="text-muted-foreground text-sm">
						Pregled ukupnog budžeta, utrošenih sredstava i raspodele po
						kategorijama.
					</p>
				</div>
			...
			<CardContent className="grid gap-6 lg:grid-cols-[320px_1fr]">
				<div className="from-primary/10 via-primary/5 to-card/80 rounded-2xl border bg-gradient-to-br p-6 shadow-inner">
					<div className="text-primary text-sm font-semibold tracking-wide uppercase">
						Ukupan budžet
					</div>
					<div className="text-primary mt-2 flex items-baseline gap-2 text-3xl font-bold">
					...
					<div className="mt-4 space-y-2 text-sm">
						...
						<div className="flex items-center justify-between">
							...
							<span className="font-medium">
								{budget.currency}{" "}
								{utilisation.remaining.toLocaleString("sr-RS")}
							</span>
						</div>
						...
						<div className="flex items-center justify-between">
							...
							<span className="font-medium">
								{((budget.totalHours ?? 0) as number).toFixed(1)}h
							</span>
						</div>
				...
				<div className="space-y-4">
					{budget.categories.length === 0 ? (
						<div className="text-muted-foreground h-full rounded-2xl border border-dashed p-8 text-center text-sm">
							Još uvek nema raspodele budžeta. Dodajte kategorije kako biste
							pratili troškove po segmentima.
						</div>
					) : (
						budget.categories.map((category, index) => {
							...
							return (
								<div
									key={category.id}
									className="bg-card/70 flex flex-col gap-3 rounded-2xl border p-4 shadow-sm transition hover:shadow-lg"
								>
									<div className="flex items-start justify-between">
										<div className="space-y-1">
											...
											<div className="text-muted-foreground text-xs tracking-wide uppercase">
												Alocirano: {budget.currency}{" "}
												{category.allocated.toLocaleString("sr-RS")}
											</div>
										</div>
										<div className="flex items-center gap-2">
											<Badge
									...
									<div className="flex flex-wrap items-center gap-3 text-sm">
										<div className="bg-muted inline-flex items-center gap-2 rounded-lg px-3 py-1">
											<Banknote className="text-muted-foreground size-4" />
											<span className="text-foreground font-medium">
												{budget.currency}{" "}
												{category.spent.toLocaleString("sr-RS")}
											</span>
			...
			<Dialog open={isBudgetDialogOpen} onOpenChange={setIsBudgetDialogOpen}>
				<DialogContent className="sm:max-w-md">
					...
					<form
						className="space-y-4"
						onSubmit={budgetForm.handleSubmit(submitBudget)}
					>
						<div className="space-y-2">
							...
							<Input
								...
								type="number"
								step="0.01"
								{...budgetForm.register("total", { valueAsNumber: true })}
							/>
						</div>
						<div className="space-y-2">
							...
							<Input
								...
								type="number"
								step="0.01"
								{...budgetForm.register("spent", { valueAsNumber: true })}
							/>
						</div>
						<div className="space-y-2">
						...
						<div className="grid gap-4 md:grid-cols-2">
							<div className="space-y-2">
								...
								<Input
									...
									type="number"
									step="0.01"
									{...categoryForm.register("allocated", {
										valueAsNumber: true,
									})}
								/>
							</div>
