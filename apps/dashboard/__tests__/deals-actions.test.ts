...
import {
	DEAL_STAGES,
	type DealStage,
} from "@/app/(protected)/crm/deals/constants";
...
describe("CRM deal actions", () => {
	...
	it("summarizes owners and stage counts", async () => {
		const ownersOrderBy = vi
			.fn()
			.mockResolvedValue([{ owner: "Alice" }, { owner: "Bob" }]);
		...
		const closedWonSummary = metadata.stages.find(
			(item) => item.stage === "Closed Won",
		);
		const negotiationSummary = metadata.stages.find(
