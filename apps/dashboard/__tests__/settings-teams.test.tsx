...
describe("Settings Teams Tab", () => {
	...
	beforeEach(() => {
		...
		fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
			...
			if (
				url.startsWith("/api/settings/team-members/") &&
				method === "DELETE"
			) {
	...
	it("renders team members list and triggers status filter requests", async () => {
		...
		await waitFor(() =>
			expect(screen.getByText("Jessica Wong")).toBeInTheDocument(),
		);
		...
		await waitFor(() => {
			expect(
				fetchMock.mock.calls.some(
					([url]) => typeof url === "string" && url.includes("status=offline"),
				),
			).toBe(true);
		});
	});
	...
	it("creates a new team member through the dialog flow", async () => {
		...
		await waitFor(() =>
			expect(screen.getByText("Ana Jovanović")).toBeInTheDocument(),
		);
	});
});
